...
import org.wby4j.framework.mvc.annotation.Request;
import org.wby4j.framework.util.ArrayUtil;
...
public class ActionHelper {
    ...
    static {
        ...
        if (CollectionUtil.isNotEmpty(actionClassList)) {
            // 定义两个 Action Map
            HashMap<Requester, Handler> commonActionMap = new HashMap<>();   // 存放普通 Action Map
            HashMap<Requester, Handler> regexpActionMap = new HashMap<>();   // 存放带有正则表达式的 Action Map
            // 遍历 Action 类
